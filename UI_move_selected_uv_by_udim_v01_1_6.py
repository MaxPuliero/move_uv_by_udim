...
class MPKeymapPreferences(AddonPreferences):
    ...
    enable_ui_widget: BoolProperty(
        name="Enable UI Widget",
        default=False,
        description="Toggle the visibility of the 'Move by UDIM' UI panel"
    )

    def draw(self, context):
        layout = self.layout
        layout.prop(self, "enable_shortcuts")
        layout.prop(self, "enable_ui_widget")
...
class UV_PT_move_udim_panel(Panel):
    ...
    def poll(cls, context):
        ...
        return preferences.enable_ui_widget
...
def register_panels():
    """Register panels based on user preferences."""
    preferences = bpy.context.preferences.addons[__name__].preferences
    if preferences.enable_ui_widget:
